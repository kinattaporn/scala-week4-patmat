...
trait Huffman extends HuffmanInterface {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    chars.groupBy(x => x).map(y => (y._1, y._2.size)).toList
  }
   ...
   */
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.sortBy(x => x._2).map(y => Leaf(y._1, y._2))
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def decodeChar(subTree: CodeTree, bits: List[Bit]): List[Char] = subTree match {
      ...
      case Fork(_, _, chars, _) if (bits.isEmpty) => chars
      case Fork(left, _, _, _) if (bits.head == 0) => {
        println("-----     bits.head", bits.head, "     left", left)
        decodeChar(left, bits.tail)
      }
      case Fork(_, right, _, _) if (bits.head == 1) => {
   ...
   */
//                                          sdxjfzkwyhqolmpurcvgbnteia
//                      sdxjfzkwyhqolmpu                                      rcvgbnteia
//  sdxjfzkwyhq                                   olmpu                rcvgbnt            eia
//  s        dxjfzkwyhq                        ol       mpu         rcvgb      nt        e   ia
//          d          xjfzkwyhq              o  l    mp    u       r  cvgb   n  t          i  a
//                xjf            zkwyhq              m  p             c   vgb
//              xj   f        zkwyh    q                                  v   gb
//             x  j          zkwy   h                                         g  b
//                         zkw   y
//                        z   kw
//                           k  w
  ...
  val secretM: List[Bit] = List(0,1,1,0,0)
  val secretA: List[Bit] = List(1,1,1,1)
  val secretN: List[Bit] = List(1,0,1,0)
  val secretE: List[Bit] = List(1,1,0)
  val secretS: List[Bit] = List(0,0,0)
  val secretT: List[Bit] = List(1,0,1,1)
  val secretC: List[Bit] = List(1,0,0,1,0)
  val secretO: List[Bit] = List(0,1,0,0)
  val secretL: List[Bit] = List(0,1,0,1)
  /**
  ...
  def convert(tree: CodeTree): CodeTable = {
    def convertBits(tree: CodeTree, bits: List[Bit]): CodeTable = tree match {
      ...
      case Fork(left, right, _, _) => convertBits(left, bits:+0) ::: convertBits(right, bits:+1)
    }
